...
def extract_normalized_eigenvector(X, condition_number: float = 5.0, n_hopframes: int = 3, is_tracking: bool = True,
    ...
    # signal to track
    # Raw Magnitude Spectrogram
    if ismag == 1:
        signal_magspec = np.abs(X[:, n_hopframes:n_hopframes + n_frames, 0])
    # Running Average Spectrogram (3 frames)
    else:
        n_autocorr_frames = 3
    ...
    for iframe in np.arange(n_hopframes, n_frames + n_hopframes):
        ...
        for ibin in np.arange(n_bins):
            if valid_bin[ibin]:
                ...
                if s[0] > s[1] * condition_number: # 5 is used
                    indicator_rank1 = True
                else:
                    indicator_rank1 = False
                # update valid bin (Pass both Magnitude and Coherence test)
                if is_tracking:
                ...
                if valid_bin[ibin]:
                    ...
                    if audio_format == 'foa':
                        # Normalized Eigenvector by first element (omni-directional channel) and discarding it
                        normed_eigenvector = np.real(u[1:, 0] / u[0, 0])
                        # Unit-norm Eigenvector-based Intensity Vector 
...
class MagStftExtractor:
    ...
    def __init__(self, n_fft: int, hop_length: int, win_length: int = None, window: str = 'hann',
        ...
        assert self.win_length <= self.n_fft, 'Window Length is greater than nfft!'
    ...
    def extract(self, audio_input: np.ndarray) -> np.ndarray:
        ...
        logmel_features = np.concatenate(log_features, axis=0)
        ...
        for split in splits:
            ...
            for count, audio_fn in enumerate(tqdm(audio_fn_list)):
                ...
                for imic in np.arange(n_mics):
                    ...
                    afeature[:, :, imic] = stft
                X = afeature[lower_bin:upper_bin, :, :] # Clip the STFT by lower and upper bins

                # Compute normalized eigenvector
                # normed_eigenvector_max ==> (Channel, Bin, Frame)
                ...
                # full_eigenvector_mat ==> (Channel, Frame, Bin)
